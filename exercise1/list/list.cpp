...
template <typename Data>
List<Data>::Node::Node(Data elem, Node* next){
...
List<Data>::Node::Node(Node&& nodo) noexcept{
    ...
    Data elem_tmp = nodo.elem;
    Node* nodo_tmp = nodo.next;

    std::swap(elem, elem_tmp);
    std::swap(next, nodo_tmp);
...
template <typename Data>
List<Data>& List<Data>::operator=(const List<Data>& elem) noexcept{
